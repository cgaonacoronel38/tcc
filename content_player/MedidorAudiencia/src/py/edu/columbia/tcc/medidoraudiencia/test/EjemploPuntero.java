...
public class EjemploPuntero extends JPanel {
    ...
    public void frametolabel(Mat matframe) {
        ...
        try {
            ...
            lbResult.setIcon(new ImageIcon(aa));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
        System.load(EjemploPuntero.class.getResource(Cons.LIBRERIA_OPENCV).getPath());

        EjemploPuntero v = new EjemploPuntero();
        VideoCapture webcam = new VideoCapture(0);
        webcam.set(Videoio.CV_CAP_PROP_FRAME_HEIGHT, 450);
        webcam.set(Videoio.CV_CAP_PROP_FRAME_WIDTH, 360);
        v.setframe(webcam);
        Robot r = new Robot();
        Mat mimm = new Mat();
        Mat modifica = new Mat();
        ...
        while (true && !close) {
            ...
            if (!webcam.isOpened() && !close) {
                ...
                if (!close && webcam.grab()) {
                    ...
                    Mat dst2 = new Mat();
                    Mat result = new Mat();
                    List<Mat> src = Arrays.asList(mimm, mimm);
                    List<Mat> src2 = Arrays.asList(mimm, mimm);
